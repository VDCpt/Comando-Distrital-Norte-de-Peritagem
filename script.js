...
const getRiskVerdict = (delta, gross) => {
    if (gross === 0 || isNaN(gross)) return { level: 'INCONCLUSIVO', key: 'low', color: '#8c7ae6', description: 'Dados insuficientes.' };
...
const translations = {
    pt: {
        startBtn: "INICIAR SESSÃO CSI v12.1",
        ...
        kpiTitle: "ANÁLISE DE TRIANGULAÇÃO · ALGORITMO CSI v12.1",
        kpiGross: "BRUTO REAL",
        kpiCommText: "COMISSÃO",
        kpiNetText: "LÍQUIDO",
        kpiInvText: "FATURA",
        consoleTitle: "LOG DE CUSTÓDIA · FORENSIC LOG",
        footerHashTitle: "INTEGRIDADE DO SISTEMA (MASTER HASH SHA-256 v12.1)",
    ...
    en: {
        startBtn: "START CSI SESSION v12.1",
        ...
        cardJuros: "TAX GAP",
        kpiTitle: "TRIANGULATION ANALYSIS · ALGORITHM CSI v12.1",
        kpiGross: "REAL GROSS",
        kpiCommText: "COMMISSION",
        kpiNetText: "NET",
        kpiInvText: "INVOICE",
        consoleTitle: "CUSTODY LOG · FORENSIC LOG",
        footerHashTitle: "SYSTEM INTEGRITY (MASTER HASH SHA-256 v12.1)",
...
function updateLoadingProgress(percent) {
    ...
    if (text) text.textContent = `FORENSIC ENGINE v12.1... ${percent}%`;
}
...
function showMainInterface() {
    ...
    logAudit('SISTEMA VDC v12.1 CSI ONLINE', 'success');
}

function loadSystemRecursively() {
    try {
        const stored = localStorage.getItem('vdc_client_data_bd_v12_1');
        if (stored) {
...
function registerClient() {
    ...
    localStorage.setItem('vdc_client_data_bd_v12_1', JSON.stringify(VDCSystem.client));
...
function activateDemoMode() {
    ...
    logAudit('ATIVANDO MODO DEMO CSI v12.1...', 'info');
    ...
    setTimeout(() => {
        ...
        logAudit('Auditoria Demo CSI v12.1 concluída com discrepância de 1.705,00€.', 'success');
...
async function exportPDF() {
    ...
    try {
        ...
        const verdict = VDCSystem.analysis.verdict || { level: 'N/A', key: 'low', description: 'Análise não executada.' };
...
function resetSystem() {
    ...
    localStorage.removeItem('vdc_client_data_bd_v12_1');
...
console.log('VDC v12.1 TERMINAL - Sistema carregado com todas as correções aplicadas.');
