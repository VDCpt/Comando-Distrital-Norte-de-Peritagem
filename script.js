...
const VDCSystem = {
    ...
    // Bolt Operations - Dados Completos (v11.5)
    boltEntity: {
...
// ===== DOMContentLoaded: EXECUÇÃO IMEDIATA =====
...
window.onload = async function() {
    try {
        ...
        if (openModalBtn) {
            ...
            console.error('❌ ERRO CRÍTICO: #openEvidenceModalBtn não encontrado no DOM!');
        }
    } catch (error) {
...
async function loadForensicSystem() {
    try {
        ...
        // Garantir que os anos estão populados
        populateYears();
...
function setupEventListeners() {
    try {
        ...
        // BOTÃO DE GESTÃO DE EVIDÊNCIAS - REPARADO E REFORÇADO
...
// ===== PROCESSAMENTO DE FICHEIROS - PARSING REGIONAL V11.5 =====
...
async function processMultipleFilesModal(type, files, appendMode = true) {
    try {
        logAudit(`📁 Processando ${files.length} ficheiros ${type.toUpperCase()} via Modal (Parsing Regional v11.5)...`, 'info');
...
// ===== MODO DEMO - COM LIMPEZA TOTAL DE ARRAYS =====
...
// ===== MOTOR DE EXTRAÇÃO - PARSING REGIONAL V11.5 =====
...
function extractSAFTData(text, filename, fileType) {
    const data = {
        ...
        records: 0,
        transactionDetails: [],
        extractionMethod: 'Parsing Regional v11.5 - Delimitador ; prioritário',
        isoStandard: 'ISO/IEC 27037'
    };
...
function extractInvoiceData(text, filename) {
    const data = {
        ...
        records: 1,
        extractionMethod: 'Parsing Regional v11.5 - Total com IVA (EUR)',
        isoStandard: 'ISO/IEC 27037'
    };
...
function extractStatementData(text, filename) {
    const data = {
        ...
        records: 0,
        transactionDetails: [],
        extractionMethod: 'Parsing Regional v11.5 - Soma de Transações Individuais',
        isoStandard: 'ISO/IEC 27037'
    };
    ...
    try {
        ...
        logAudit(`🏦 Extrato ${filename}: Bruto=${data.grossEarnings.toFixed(2)}€ | Comissão=${data.commission.toFixed(2)}€ | Campanhas=${data.campaigns.toFixed(2)}€ | Gorjetas=${data.tips.toFixed(2)}€ | ${data.records} registos (Parsing Regional v11.5)`, 'success');
    } catch (error) {
...
async function exportJSON() {
    try {
        ...
        const evidenceData = {
            sistema: "VDC Forensic System v11.5 - Auditoria Fiscal Big Data 'THE CHECKMATE'",
...
function exportPDF() {
    try {
        ...
        if (typeof window.jspdf !== 'undefined' && window.jspdf.jsPDF) {
            ...
            pdf.text('VDC FORENSIC SYSTEM v11.5 - RELATÓRIO PERICIAL "THE CHECKMATE"', pageWidth / 2, yPos, { align: 'center' });
            yPos += 10;
            ...
            pdf.text('Sistema de Peritagem Forense VDC v11.5 "THE CHECKMATE" - ISO/IEC 27037 | NIST SP 800-86 | © 2024', pageWidth / 2, pageHeight - 10, { align: 'center' });
...
function resetDashboard() {
    try {
        ...
        localStorage.removeItem('vdc_clients_bd_v11_5');
        VDCSystem.preRegisteredClients = [];
...
function updatePageTitle(status) {
    const baseTitle = 'VDC | Sistema de Peritagem Forense v11.5';
    document.title = status ? `${baseTitle} - ${status}` : baseTitle;
}

// ===== EXPORTAÇÃO GLOBAL DE FUNÇÕES (V11.5 - PROTOCOLO DE FECHO) =====
